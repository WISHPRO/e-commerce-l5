...
class BackendCountiesController extends Controller
{
    ...
    public function store( Request $request )
    {
        ...
        if (!$county->validate()) {
            return Redirect::back()->withErrors( $county->errors() )->withInput()->with(
                'message',
                $this->FormErrorMsg
            )->with( 'alertclass', 'alert-danger' );
        }

        return $county->save() ? Redirect::route( 'counties.view' )->with( 'message', $this->successMsg )->with(
            'alertclass',
            'alert-success'
        ) : Redirect::back()->withErrors( $county->errors() )->withInput()->with(
            'message',
            'Adding the county failed because some errors occurred. please fix them'
        )->with( 'alertclass', 'alert-danger' );
    }
    ...
    public function update( $id )
    {
        ...
        return Redirect::route( 'counties.view' )->with(
            'message',
            'successfully updated county with id ' . $id
    ...
    public function destroy( $id )
    {
        ...
        return Redirect::route( 'counties.view' )->with(
            'message',
            'successfully deleted county with id ' . $id
        )->with( 'alertclass', 'alert-success' );
    }
