...
trait DiscountsTrait
{
    ...
    public function getPriceAfterDiscount($format = true, $returnMoneyInstance = false)
    {
        ...
        if ($format) {
            return format_money($this->getPriceAfterTax($this)->subtract($value->toMoney()));
        }

        return $returnMoneyInstance ? $this->getPriceAfterTax($this)->subtract($value->toMoney()) : $this->getPriceAfterTax($this)->subtract($value->toMoney())->getAmount();
    }
